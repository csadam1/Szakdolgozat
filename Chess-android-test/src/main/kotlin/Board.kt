class Board {
    ...
    /**
     * Executes the player's move and updates the board accordingly.
     */
    fun makeMove(move: Move) {
        // Decides if it's a capture move
        var capture = getSpotById(move.to).piece::class != Empty::class

        // Get the piece from the 'move.from' square. Saving it in a temporary variable
        val movingPiece = getSpotById(move.from).piece
        // Clear the spot
        getSpotById(move.from).piece = Empty()
        // Move the piece to 'to' spot
        getSpotById(move.to).piece = movingPiece

        // If it's an En Passant capture move, the other player's piece is set to empty
        if (isEnPasMove(move.to, movingPiece)) {
            capture = true
            captureEnPas(move)
        }
        // If it's a promotion move, replace the current pawn with the selected piece
        if (isPromotionMove(move)) {
            promotePawn(getSpotById(move.to), move.promotion!!)
        }
        // If it's a castling move, replace the rook to the correct position
    ...
    /**
     * If it's a castling move, replaces the rook based on where the King made the castling move.
     */
    ...
    /**
     * Checks if the move is promotion move
     */
    ...
    /**
     * Decides if the move was an En Passant capture move.
     */
    ...
    /**
     * The function removes the opponent's piece from an En Passant move
     */
    ...
    /**
     * Resets half moves if it's a pawn move or a piece is captured, otherwise it's value is incremented by 1.
     */
